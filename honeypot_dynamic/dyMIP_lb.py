...
if __name__ == "__main__":
    ...
    score_list = []
    start_time = timeit.default_timer()
    #hasmask_dir = "/hpcfs/users/a1798528/honeypot_dynamic_HPC/processed_dygraph/" + args["fn"] + "_hasmask" + ".pkl"
    #with open(hasmask_dir, 'rb') as f:
    #    data = pickle.load(f)
    hasmask = None
    start = args["thread_number"]*args["graph_number"]
    ...
    data = []
    print(len(hasmask))
    competent_score_list, flat_score_list, blocking_list = dyMIP(CG, args["batch_number"], args["graph_number"], args["spacing_window"], hasmask, args["phi"])
    time = timeit.default_timer() - start_time
